...
then
	...
	filt_gtf_name=${filt_gtf_name/".gtf"/".${fet}.gtf"}
	...
	if [ $lc -eq 0 ]
        then
                echo -e "${RED} your input feature (${fet}) doesnot exist in ${sp} gtf file ${NC}"
		rm $filt_gtf_path
		exit 128
	else
		echo -e "${GREEN} filtered record for ${fet} in ${filt_gtf_name} is ${lc} ${NC}"
	fi
	...
	awk -v sp="${sp}" -v fet="${fet}" 'BEGIN{FS="\t"; OFS=","; print "species,"fet"_avg_gc,"fet"_avg_len"} { total_gc += $11; count_gc++; total_len +=$18; count_len++ } \
