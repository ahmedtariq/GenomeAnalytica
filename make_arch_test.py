...
def plot_sig(y_list ,df,df_test,output,rank_value,x = 'Flag'):
    ...
    Rows = Tot // Cols
    Rows += Tot % Cols
    Rows = Rows if (Tot % Cols) == 0  else Rows + 1
    ...
    for i,c in enumerate(y_list):
        ...
        ax.set_title("ttest_adj_p = " + str(round(df_test.loc[df_test['feature'] == c,'ttest_adj_p'].item(),5)) + " Mann_adj_p = " + str(round(df_test.loc[df_test['feature'] == c,'MannWhitney_adj_p'].item(),5)))
...
def make_test(args):
    ...
    # joining with parsed data and flagging user specified tax value as true:
    ...
    # creating figure and saving it to desk

    plot_sig(y_list = df_test[(df_test['ttest_adj_reject'] == True) | (df_test['MannWhitney_adj_reject'] == True) ]['feature'].to_list(),df=df,df_test=df_test,output=out_fig,rank_value=rank_value)
...
if __name__=="__main__":
    ...
    Box plot figure for adj_ttest OR adj_MannWhetney significant features
    """
    ...
    parser.add_argument('--stats',required=True, nargs='?',type=str, help='Directory containning the nested csv stats for all species in form of (species,stats)')
    parser.add_argument('--tax', required=True, nargs='?',type=str, help='File csv contianing the full lineage of all species in stats')
    ...
    parser.add_argument('--tax_value', required=True, nargs='?',type=str, default="Vertebrata", help='value to test for in the tax rank e.g: Vertebrata to test Vert vs In-Vert in Subphylum')
